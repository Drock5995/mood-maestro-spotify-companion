...
function DashboardContent() {
  ...
  // Fetch shared playlists for the user
  useEffect(() => {
  ...
  useEffect(() => {
    const fetchTopArtists = async () => {
      if (spotifyApi) {
        ...
        try {
          const artists = await spotifyApi.getUserTopArtists(5); // Get top 5 artists
  ...
  // Calculate total songs and top genres from top artists
  const totalSongs = useMemo(() => {
    return playlists.reduce((sum, playlist) => sum + playlist.tracks.total, 0);
  }, [playlists]);
  ...
  return (
    <>
      <svg width="0" height="0" style={{ position: 'absolute' }}><defs>{gradients.map((grad, index) => (
        <linearGradient id={`dashboardColor${index}`} x1="0" y1="0" x2="1" y2="0" key={index}><stop offset="0%" stopColor={grad[0]} /><stop offset="100%" stopColor={grad[1]} /></linearGradient>
      ))}</defs></svg>

      <header className="mb-8">
        <h1 className="text-4xl sm:text-5xl font-extrabold text-white mb-2">
          Welcome, <span className="text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-pink-500">{user?.display_name || 'VibeSphere User'}</span>!
        </h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
        <motion.div 
          className="bg-white/5 p-6 rounded-xl flex items-center space-x-4"
        ...
        >
          ...
          <div>
            ...
            <h3 className="text-3xl font-bold text-white">{playlists.length}</h3>
          </div>
        </motion.div>
        <motion.div 
          className="bg-white/5 p-6 rounded-xl flex items-center space-x-4"
        ...
        >
          ...
          <div>
            ...
            <h3 className="text-3xl font-bold text-white">{totalSongs}</h3>
          </div>
        </motion.div>
        <motion.div 
          className="bg-white/5 p-6 rounded-xl flex flex-col"
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.5, delay: 0.3 }}
        >
          <div className="flex items-center space-x-4 mb-2">
            <BarChart2 className="w-8 h-8 text-blue-400 flex-shrink-0" />
            <h3 className="text-xl font-bold text-white">Top Artists</h3>
          </div>
          {loadingTopArtists ? (
            <div className="flex-1 flex items-center justify-center"><div className="animate-spin rounded-full h-6 w-6 border-t-2 border-b-2 border-blue-500"></div></div>
          ) : (
            <ul className="text-gray-300 text-sm space-y-1">
              {topArtists.map((artist, idx) => (
                <li key={artist.id} className="truncate">{idx + 1}. {artist.name}</li>
              ))}
            </ul>
          )}
        </motion.div>
        ...
        >
          <div className="flex items-center space-x-4 mb-2">
            <BarChart2 className="w-8 h-8 text-emerald-400 flex-shrink-0" />
            <h3 className="text-xl font-bold text-white">Top Genres</h3>
          </div>
          {loadingTopArtists ? (
            <div className="flex-1 flex items-center justify-center"><div className="animate-spin rounded-full h-6 w-6 border-t-2 border-b-2 border-emerald-500"></div></div>
          ) : (
            <div style={{ width: '100%', height: 100 }}>
              <ResponsiveContainer>
                <BarChart data={topGenres} layout="vertical" margin={{ top: 0, right: 0, left: 0, bottom: 0 }}>
                  <XAxis type="number" hide />
                  <YAxis type="category" dataKey="name" hide />
                  <Tooltip cursor={{ fill: 'rgba(255,255,255,0.1)' }} contentStyle={{ backgroundColor: '#1A202C', border: 'none', borderRadius: '10px' }} />
                  <Bar dataKey="count" barSize={10} radius={[0, 5, 5, 0]}>
                    {topGenres.map((entry, index) => (
                      <Cell key={`cell-${index}`} fill={`url(#dashboardColor${index % gradients.length})`} />
                    ))}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
